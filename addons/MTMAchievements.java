...
import clashsoft.mods.moretools.item.tools.ItemHoeMTM;
...
public class MTMAchievements
{
	...
	public static class MoreToolsModCraftingHandler implements ICraftingHandler
	{
		...
		public void onCrafting(EntityPlayer player, ItemStack item, IInventory craftMatrix)
		{
			...
			if (item.getItem() instanceof ItemToolMTM || item.getItem() instanceof ItemSwordMTM || item.getItem() instanceof ItemHoeMTM || item.getItem() instanceof ItemArmorMTM)
			{
				player.triggerAchievement(moretools);
				
				if (item.getItem() instanceof ItemToolMTM)
					player.triggerAchievement(materialachievements.get(((ItemToolMTM) item.getItem()).toolMaterial));
				else if (item.getItem() instanceof ItemSwordMTM)
					player.triggerAchievement(materialachievements.get(((ItemSwordMTM) item.getItem()).toolMaterial));
				else if (item.getItem() instanceof ItemHoeMTM)
					player.triggerAchievement(materialachievements.get(((ItemHoeMTM) item.getItem()).theToolMaterial));
				else if (item.getItem() instanceof ItemArmorMTM)
					player.triggerAchievement(materialachievements.get(((ItemArmorMTM) item.getItem()).material));
			}
			if (item.getItem() instanceof ItemMTM)
			{
				player.triggerAchievement(moretools);
			}
		}
